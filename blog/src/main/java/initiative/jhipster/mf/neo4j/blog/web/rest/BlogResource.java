...
import initiative.jhipster.mf.neo4j.blog.repository.BlogRepository;
...
/**
 * REST controller for managing {@link initiative.jhipster.mf.neo4j.blog.domain.Blog}.
 */
@RestController
...
public class BlogResource {
    ...
    public Mono<ResponseEntity<Blog>> createBlog(@Valid @RequestBody Blog blog) throws URISyntaxException {
        ...
        return blogRepository.save(blog).map(result -> {
            try {
    ...
    public Mono<ResponseEntity<Blog>> updateBlog(
        ...
        return blogRepository.existsById(id).flatMap(exists -> {
            if (!exists) {
