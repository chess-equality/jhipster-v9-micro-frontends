...
import initiative.jhipster.mf.neo4j.blog.repository.TagRepository;
import initiative.jhipster.mf.neo4j.blog.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link initiative.jhipster.mf.neo4j.blog.domain.Tag}.
 */
@RestController
...
public class TagResource {
    ...
    public Mono<ResponseEntity<Tag>> createTag(@Valid @RequestBody Tag tag) throws URISyntaxException {
        ...
        return tagRepository.save(tag).map(result -> {
            try {
    ...
    public Mono<ResponseEntity<Tag>> updateTag(@PathVariable(value = "id", required = false) final String id, @Valid @RequestBody Tag tag)
        ...
        return tagRepository.existsById(id).flatMap(exists -> {
            if (!exists) {
    ...
    public Mono<ResponseEntity<Tag>> partialUpdateTag(
        ...
        return tagRepository.existsById(id).flatMap(exists -> {
            ...
            Mono<Tag> result = tagRepository
                .findById(tag.getId())
                .map(existingTag -> {
                    updateIfPresent(existingTag::setName, tag.getName());
